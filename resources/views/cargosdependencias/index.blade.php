...
<div class="col-lg-6 col-12 mx-auto">
      ...
      <br>
       <br>
            <a type="button"href="{{ route('cargosdependencias.create') }}"class="bg-indigo-500 px-12 py-2 rounded text-gray-200 font-semibold hover:bg-indigo-800 transition duration-200 each-in-out text-center ">Agregar</a>
        </div>
...
<div class="col-sm-12">
  <table id="Registros" class="table table-bordered table-striped dataTable dtr-inline">
    ...
    <tbody>
      @foreach ($cargosdependencias as $cargosdependencia)
      <tr>
        ...
        <td class="px-4 py-2">
          <div class="flex justify-center space-x-2">
            ...
            <a href="{{ route('cargosdependencias.edit', $cargosdependencia->id_dependencia) }}" class="rounded bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4">Editar</a>
            <!-- botón borrar -->
           <form action="{{ route('cargosdependencias.destroy', ['id1' => $cargosdependencia->id_dependencia, 'id2' => $cargosdependencia->id_cargo]) }}" method="POST" class="formEliminar">
    @csrf
    @method('DELETE')
  ...
  Array.prototype.slice.call(forms)
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {        
          ...
          Swal.fire({
                title: '¿Confirma la eliminación de la cargo?',        
                icon: 'info',
