  ...
  <table id="zonas" class="table table-bordered table-striped dataTable dtr-inline">
    ...
    <tbody>
      ...
      <tr>
         ...
         <td class="px-4 py-2 text-center">{{$zona->circuito->nombre}}</td>
        <td class="px-4 py-2 text-center">{{$zona->nombre}}</td>
        <td class="px-4 py-2">
          <div class="flex justify-center space-x-2">
            ...
            <!-- botón borrar -->
            <form action="{{ route('zonas.destroy', $zona->id ) }}" method="POST" class="formEliminar">
              @csrf
              @method('DELETE')
